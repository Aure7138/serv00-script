...
if [ -d "$xray_dir" ]; then
    # 添加新的保活任务
    (crontab -l 2>/dev/null; echo "*/5 * * * * if ! pgrep -f \"./xray run -c config.json\" > /dev/null; then cd $xray_dir && nohup ./xray run -c config.json > /dev/null 2>&1 & fi") | crontab -
...
if [ -d "$hysteria2_dir" ]; then
    (crontab -l 2>/dev/null; echo "*/5 * * * * if ! pgrep -f \"./hysteria-freebsd-amd64 server -c config.yaml\" > /dev/null; then cd $hysteria2_dir && nohup ./hysteria-freebsd-amd64 server -c config.yaml > /dev/null 2>&1 & fi") | crontab -
